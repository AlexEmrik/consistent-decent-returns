...
class RidgeRegressor(AlphaPredictor):
    ...
    def fit(self, xt: np.ndarray, yt: np.ndarray) -> np.ndarray:    
        ...
        if self.const == True:
            ...
            self.mu = np.nanmean(xt, axis=0, keepdims=True)
            self.sd = np.nanstd(xt, axis=0, keepdims=True) + 1e-8
        ...
        if self.pooled == True:
            xw = xw.reshape(TN, F)
            yw = yw.reshape(TN)
            a = xw.T @ xw + self.gamma * I_f
            b = xw.T @ yw

            theta = np.linalg.solve(a, b)  # [F,]
        else:
    ...
    def predict(self, xt: np.ndarray) -> np.ndarray:
        ...
        if self.const == True:
            xt.copy()
            xt[:, :, 1:] = (xt[:, :, 1:] - self.mu) / self.sd # xt[:, 1:] = standard_scale(xt[:, 1:], axis=0)
        else:
            xt = (xt - self.mu) / self.sd # xt = standard_scale(xt, axis=0)
